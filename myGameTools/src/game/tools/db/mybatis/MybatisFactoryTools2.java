 ...
 * @author zhibing.zhou
 */
class MybatisFactoryTools2 {

	/** 2017年4月22日 下午1:42:31			 mysql driver字符串*/
	private static final String MYSQL_DRIVER = "com.mysql.jdbc.Driver";
	
	/** 2017年2月16日 上午11:13:35 mybatis的session列表集合<数据源Key， 数据源会话对象>			 */
	private static final ConcurrentHashMap<Object, SqlSessionTemplate> MYBATIS_SESSION_MAP = new ConcurrentHashMap<Object, SqlSessionTemplate>();

	/**
	 * 第一个注册的数据编号
	 */
	private static Object FIRST_SESSION_KEY;
	 ...
	 * 程序项目运行的路径
	 */
	private static String RUN_PROJECT_PATH;
	

	private static DataSource createDataSource(String username, String password, String jdbcUrl)
	{
		return createDbcpDataSource(username, password, jdbcUrl, 10, 500, 5000);
	}

	private static DataSource createDbcpDataSource(String username, String password, String jdbcUrl, int initialSize,int maxActive, int maxWait) 
	{
...
//		dds.getDefaultAutoCommit();
	...
	private static DataSource createDruidDataSource(String username, String password, String jdbcUrl, int initialSize,int maxActive, int maxWait) 
	{
	...
	private static String getProjectPath() 
	{
		...
		URL url = MybatisFactoryTools2.class.getClassLoader().getResource("");
		if(url == null)
			url = MybatisFactoryTools2.class.getResource("");
...
//		System.out.println("realPath1 = " + realPath);
	...
	private static String getRunJarPath(String rootMapper) 
	{
...
//		System.out.println("publish = " +publish+ " urlString = " + urlString );
	 ...
	 */
	private static ArrayList<Class> getMapperClassList(String rootMapper) 
	{
	 ...
	 */
	private static SqlSessionFactory createSqlSessionFactory(DataSource dataSource, ArrayList<Class> mapperList) 
	{
...
//		new DataSourceTransactionManager().setDataSource(dataSource);
	...
	private static SqlSessionFactory createSqlSessionFactory(String rootMappers, String username, String password,String url) 
	{
	...
	private static void registerMyBatisFactory(Object sessionNo, String rootMappers, String url, String username, String password, Interceptor... interceptor) 
	{
		if(sessionNo == null)
		{
			try {
...
//	public static SqlSessionFactory getSessionFactory(Object sessionNo)
//	{
...
//		return session.getMapper(mapperClass);
	...
	private static void createThradPool() 
	{
		if (THREAD_POOL == null)
...
//	}
	...
	public static ArrayList<Result> executeAll(MybatisFactoryCmd cmd) 
	{
		...
		{
			ForkJoinTools forkJoinTools = new ForkJoinTools();
	 ...
	 */
	private static Object getRandomMybatisFactoryDbNo() 
	{
		if(MYBATIS_SESSION_MAP.size() == 1)
	...
	private static ArrayList<Result> executeReturnNew(MybatisFactoryCmd cmd) 
	{
	 ...
	 */
	private static ArrayList<Class> getJarFileClassList(String rootMapper) 
	{
...
//		getJarPath("game.data.conf.mapper");
		...
		MybatisFactoryTools2.registerMyBatisFactory("game.data.conf.mapper", "jdbc:mysql://192.168.1.131:3306/game_logic_conf" ,
				"innertest01", "innertest01");
		MybatisFactoryTools2.registerMyBatisFactory("game.data.conf.mapper", "jdbc:mysql://192.168.1.131:3306/game_logic_conf" ,
				"innertest01", "innertest01");
		
		
		List<Result> listt = MybatisFactoryTools2.executeAll(new MybatisFactoryCmd<PrfWarZoneTitleMapper>() {
...
//			});
			...
			List<Result> list2 = MybatisFactoryTools2.executeAll(new MybatisFactoryCmd<PrfWarZoneTitleMapper>() {
...
//		 int no = mapper.insert(wt);
	...
	private static SubForkTask getSubForkTask(Object dbNo, Class cls  , MybatisFactoryCmd cmd  )
	{
